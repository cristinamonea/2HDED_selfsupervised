...
class TrainModel():
    ...
    def initialize(self, opt):
        ...
        self.checkpoints_path = os.path.join(self.opt.checkpoints,
                                             self.opt.name)
        self.scheduler = None
        # self.depth_loss = opt.depth_loss
        # self.aif_loss = opt.aif_loss
        # self.depth_loss_coef = opt.depth_loss_coef
        # self.aif_loss_coef = opt.aif_loss_coef
    ...
    def get_optimizerG(self, network, lr, weight_decay=0.0):
        ...
        return optim.Adam(generator_params, lr=lr, betas=(self.opt.beta1, self.opt.beta2), weight_decay=weight_decay)
    ...
    def train(self, data_loader, val_loader=None):
        ...
        for epoch in range(self.start_epoch, self.opt.nEpochs):
            ...
            for self.it in self.pbar:
                ...
                # Update progress bar description (optional, for detailed info)
           ...
           # self.update_learning_rate(epoch)
            if self.opt.optim != None and self.update_lr:
                self.scheduler.step()
                print("Current optimizer params: ", self.optimG)    
            #step schedular
            # print(self.lr_sc)
    ...
    def apply_valid_pixels_mask(self, *data, value=0.0):
        ...
        mask = (data[1].data > value).to(self.device, dtype=torch.float32)
