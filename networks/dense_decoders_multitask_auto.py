...
from .conv_blocks import get_decoder_block, conv3x3, conv4x4, UpsampleBlock, ScaledSigmoid
...
class DenseUNet_aif(nn.Module):
    ...
    def __init__(self, d_block, input_nc=3, outputs_nc=[1], growth_rate=32,
        ...
        self.decoder_aif.add_module('last_tanh_layer', nn.Tanh())
    ...
    def forward(self, x):
        ...
        # if 'depth' in self.tasks:
            ...
            # input is (ngf * 2) x 64 x 64
            # out_d2 = self.decoder_tasks.taski_block2()
        out_reg_d2 =self.decoder_tasks[0].d_block2(self.get_decoder_input(tb_denseblock1, out_d3))
            # input is ngf x 128 x 128
        out_reg_d1 =self.decoder_tasks[0].d_block1(self.get_decoder_input(out_conv1, out_reg_d2))
            # input is ngf x 256 x 256
        out_reg = self.decoder_tasks[0].last_conv(out_reg_d1)
        out_reg = self.decoder_tasks[0].last_sigmoid_layer(out_reg)
        depth = out_reg
        ...
        out_reg = self.decoder_aif.last_tanh_layer(out_reg)
